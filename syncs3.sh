...
LOGFILE="/home/user/Desktop/autotest_run_count.txt"
...
while [ 1 ] ; do
	...
	if [ $HOUR -ge 7 -a $HOUR -lt 12 ] ; then #do it's job between 10PM and 7AM 
	#run program
	echo "1hi $DATE" 
	...
	echo $RUNS > $LOGFILE
		...
		elif [ $HOUR -ge 12 -a $HOUR -lt 13 ] ; then #do it's job between 10PM and 7AM
		# run program
		echo "2hi $DATE" 
		#aws s3 sync $SYNC_DIR --region=us-east-1 s3://98backup/99_server_bkp$SYNC_DIR
		RUNS=$((RUNS+9))
